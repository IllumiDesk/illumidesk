...
@pytest.mark.asyncio
async def test_add_announcement__call_fetch_with_correct_url(jupyterhub_api_environ, make_http_response, make_mock_request_handler):
    ...
    with patch.object(AsyncHTTPClient, 'fetch', return_value=make_http_response(handler=local_handler.request)) as mock_client:
