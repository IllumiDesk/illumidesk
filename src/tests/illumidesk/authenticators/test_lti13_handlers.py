    ...
    with patch.object(LTIUtils, 'convert_request_to_dict', return_value=lti13_auth_params_dict):
        with patch.object(LTI13LaunchValidator, 'validate_login_request', return_value=True):
            with patch.object(
                ...
                with patch.object(
                    LTI13LoginHandler, 'authorize_redirect', return_value=None) as mock_auth_redirect:
...
@pytest.mark.asyncio
async def test_lti_13_login_handler_sets_state_with_next_url_obtained_from_target_link_uri(monkeypatch, lti13_login_params, make_mock_request_handler):
    """
    Do we get the expected nonce value result after hashing the state and returning the
    hexdigest?
    """
    monkeypatch.setenv('LTI13_AUTHORIZE_URL', 'http://my.lms.platform/api/lti/authorize_redirect')
    lti13_login_params['target_link_uri'] = [(lti13_login_params['target_link_uri'][0].decode() + '?next=/user-redirect/lab').encode()]
    local_handler = make_mock_request_handler(LTI13LoginHandler)
    ...
    with patch.object(LTIUtils, 'convert_request_to_dict', return_value=decoded_dict):
        with patch.object(LTI13LaunchValidator, 'validate_login_request', return_value=True):
            with patch.object(
                LTI13LoginHandler, 'authorize_redirect', return_value=None) as mock_auth_redirect:
                expected_state_json = {"state_id": "6f0ec1569a3a402dac61626361d0c125", "next_url": "/user-redirect/lab"}
