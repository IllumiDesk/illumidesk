...
def custom_auth_state_hook(spawner: Spawner, auth_state: dict) -> None:
    ...
    spawner.log.debug(
        "Assigned USER_ROLE env var to %s" % spawner.environment["USER_ROLE"]
    )
    ...
    shared_folder_allowed = (
        True
        if not user_is_an_instructor(user_role)
        else spawner.load_shared_folder_with_instructor
    )
    if (
        shared_folder_enabled.lower() in ("true", "1")
        and course_id
        and shared_folder_allowed
    ):
        spawner.log.debug("Adding the shared folder for %s" % course_id)
        spawner.volumes[f"{mnt_root}/{org_name}" + "/shared/" + course_id] = (
            notebook_dir + "/shared"
        )
