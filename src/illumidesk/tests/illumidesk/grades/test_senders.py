...
class TestLTI11GradesSender:
    ...
    @pytest.mark.asyncio
    async def test_grades_sender_raises_an_error_if_assignment_not_found_in_control_file(
        self, tmp_path
    ):
        """
        ...
        with patch.object(
            ...
            return_value=(lambda: 10, grades_nbgrader),
        ):
...
class TestLTI13GradesSender:
    def test_sender_sets_lineitems_url_with_the_value_in_auth_state_dict(
        ...
        assert (
            sut.course.lms_lineitems_endpoint
    ...
    async def test_sender_raises_AssignmentWithoutGradesError_if_there_are_no_grades(
        ...
        sut = LTI13GradeSender("course-id", "lab")
        with patch.object(
            LTI13GradeSender, "_retrieve_grades_from_db", return_value=(lambda: 10, [])
        ):
            with pytest.raises(AssignmentWithoutGradesError):
    ...
    async def test_sender_calls_set_access_token_header_before_to_send_grades(
        ...
        with patch(
            "illumidesk.grades.senders.get_lms_access_token",
    ...
    @pytest.mark.parametrize(
        "http_async_httpclient_with_simple_response", [[]], indirect=True
    )
    async def test_sender_raises_an_error_if_no_line_items_were_found(
        ...
        with patch(
            ...
            with patch.object(
                LTI13GradeSender,
                "_retrieve_grades_from_db",
                return_value=(lambda: 10, [{"score": 10}]),
            ):
                with pytest.raises(GradesSenderMissingInfoError):
                    await sut.send_grades()
