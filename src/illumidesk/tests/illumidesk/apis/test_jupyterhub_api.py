...
def test_initializer_sets_headers_to_make_request(jupyterhub_api_environ):
    ...
    assert (
        sut.default_headers["Authorization"]
        == f'token {os.environ.get("JUPYTERHUB_API_TOKEN")}'
    )
...
@pytest.mark.asyncio
async def test_add_student_to_jupyterhub_group_raises_error_when_empty(
    ...
    """
    with pytest.raises(ValueError):
        await JupyterHubAPI().add_student_to_jupyterhub_group(
            course_id="", student="foo"
        )

    with pytest.raises(ValueError):
        await JupyterHubAPI().add_student_to_jupyterhub_group(
            course_id="abc", student=""
        )


@pytest.mark.asyncio
async def test_add_instructor_to_jupyterhub_group_raises_error_when_empty(
    jupyterhub_api_environ,
):
    """
    ...
    """
    with pytest.raises(ValueError):
        await JupyterHubAPI().add_instructor_to_jupyterhub_group(
            course_id="", instructor="foo"
        )

    with pytest.raises(ValueError):
        await JupyterHubAPI().add_instructor_to_jupyterhub_group(
            course_id="abc", instructor=""
        )


@pytest.mark.asyncio
async def test_add_user_to_jupyterhub_group_raises_error_when_empty(
    jupyterhub_api_environ,
):
    """
    ...
    """
    with pytest.raises(ValueError):
        await JupyterHubAPI()._add_user_to_jupyterhub_group(
            username="", group_name="foo"
        )

    with pytest.raises(ValueError):
        await JupyterHubAPI()._add_user_to_jupyterhub_group(
            username="abc", group_name=""
        )
...
@pytest.mark.asyncio
@patch("illumidesk.apis.jupyterhub_api.JupyterHubAPI._request")
async def test_create_users_uses_request_helper_method_with_correct_values(
    mock_request, jupyterhub_api_environ
):
    """
    ...
    mock_request.assert_called_with(
        "users", method="POST", body=json.dumps(body_usernames)
    )
...
async def test_add_group_member_uses_request_helper_method_with_correct_values(
    ...
    mock_request.assert_called_with(
        "groups/to_group/users", method="POST", body=json.dumps(body_usernames)
    )
