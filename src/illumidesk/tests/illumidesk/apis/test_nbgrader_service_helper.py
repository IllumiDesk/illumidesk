...
class TestNbGraderServiceHelper:
    def test_nbgrader_format_db_url_method_uses_env_vars_to_get_db_url(
        self, monkeypatch
    ):
        monkeypatch.setattr(
            "illumidesk.apis.nbgrader_service.nbgrader_db_host", "test_host"
        )
        monkeypatch.setattr(
            "illumidesk.apis.nbgrader_service.nbgrader_db_password", "test_pwd"
        )
        monkeypatch.setattr(
            "illumidesk.apis.nbgrader_service.nbgrader_db_user", "test_user"
        )
        monkeypatch.setattr(
            "illumidesk.apis.nbgrader_service.nbgrader_db_name", "test_db"
        )
        
        monkeypatch.setattr("illumidesk.apis.nbgrader_service.org_name", "org-dummy")

        assert (
            nbgrader_format_db_url() == "postgresql://test_user:test_pwd@test_host:5432/test_db"
        )
